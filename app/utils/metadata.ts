...
export async function generateMetadata({ params }: { params: { mdxPath?: string[] } }) {
  const title = params?.mdxPath?.join(" ") || "Project Genius Hub — Zaria";
  ...
  let description = "Project Genius Hub Zaria supports community-centred problem discovery, rapid prototyping and evidence-based solutions.";
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      url,
      siteName: "Project Genius Hub — Zaria",
      images: [{ url: image }],
      type: "website",
    },
    twitter: {
